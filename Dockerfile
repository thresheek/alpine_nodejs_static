...
RUN cd node-$NodeVersion && ./configure --dest-cpu=$CpuVersion --fully-static && make -j$(getconf _NPROCESSORS_ONLN)
...
RUN tar -czvf node-$NodeVersion-alpine-$CpuVersion.tar.gz ./bin ./LICENSE && rm -rf ./bin ./LICENSE
RUN cp ./node-$NodeVersion-alpine-$CpuVersion.tar.gz /node_staging/node-$NodeVersion-alpine-$CpuVersion.tar.gz && ls -l /node_staging
